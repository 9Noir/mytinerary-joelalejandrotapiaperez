...
export default function GoogleSignIn() {
    ...
    const handleCredentialResponse = async (response) => {
        setIsLoading(true), dispatch(signinGoogle({ token_id: response.credential }));
    };
    ...
    useEffect(() => {
        ...
        let timer;
        const checkAndInitialize = () => (window.google ? initializeGoogleSignIn() : (timer = setTimeout(checkAndInitialize, 1000)));
        checkAndInitialize();
    ...
    return (
        <>
            <button onClick={handleGoogleLogin} type="button" className="active:scale-95 transition-all flex gap-4 items-center justify-center w-full border bg-slate-50 px-4 py-3 rounded-full font-medium text-neutral-500 dark:text-neutral-300 shadow-md dark:bg-white/20 hover:brightness-[.98] dark:hover:brightness-[.9] border-neutral-300 dark:border-neutral-500 ">
