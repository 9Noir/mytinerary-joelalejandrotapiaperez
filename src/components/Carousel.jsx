    ...
    <div className="flex flex-col md:w-2/3 min-h-[12rem] grow justify-between items-center relative gap-6">
      ...
      <div className="flex w-full justify-center items-center relative">
        ...
        <div className="absolute w-full justify-between flex hover:[&>*]:bg-white/50 [&>*]:px-4 [&>*]:py-2 [&>*]:rounded-full text-gray-500 text-5xl">
          <button
            onClick={previous}
            className="fa-solid fa-chevron-left active:animate-ping"
          ></button>
          <button
            onClick={fordward}
            className="fa-solid fa-chevron-right active:animate-ping"
          ></button>
        </div>
      </div>
      <div className="text-5xl text-gray-400 gap-1 flex">
        <button
          ...
          className={`fa-solid fa-minus ${index === 0 && "text-gray-500"}`}
        ></button>
        <button
          onClick={() => dotClickHandler(3)}
          className={`fa-solid fa-minus ${index === 3 && "text-gray-500"}`}
        ></button>
        <button
          onClick={() => dotClickHandler(6)}
          className={`fa-solid fa-minus ${index === 6 && "text-gray-500"}`}
        ></button>
      </div>
    </div>
  );
